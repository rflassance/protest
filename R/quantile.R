...
#'
#' @examples
#' quant_dist(pdist = pexp, qdist = qexp, x0 = 1, p0 = 0.5)
#'
#' @export
quant_dist <- function(pdist, qdist, x0 = 0, p0 = 0.5) {
  f_quant <- qdist(p = p0)
  a <- min(f_quant, x0)
  b <- max(f_quant, x0)
  dissim <- stats::integrate(
    function(x) abs(p0 - pdist(x)),
    lower = a, upper = b
  )$value
  return(dissim)
}
...
#'
#' @export
quant_test <- function(p_list, q_list, x0 = 0, p0 = 0.5, alpha = 0.05, epsilon,
                       verbose = TRUE, plot = FALSE) {
  dissim <- sapply(
    1:length(p_list), # nolint
    function(i) {
      protest::quant_dist(p_list[[i]], q_list[[i]],
        x0 = 0, p0 = 0.5
      )
    }
  )
