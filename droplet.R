...
s <- c(
  4.7, 8.1, 11.5, 14.5, 17.4, 19.9, 22.4, 24.6, 26.6, 28.5, 30.1, 31.5,
  32.8, 33.8, 34.6
)
delta_s <- c(NA, diff(s))
...
lm_res <- protest::lm_test(f_list1,
  X = cbind(1, x),
...
lm_res <- protest::lm_test(f_list2,
  X = cbind(1, x),
  alpha = 0.05, epsilon = epsilon / sqrt(length(x)),
  verbose = TRUE, plot = TRUE
)
...
lm_res <- protest::lm_test(f_list3,
  X = cbind(1, x),
  alpha = 0.05, epsilon = epsilon / sqrt(length(x)),
  verbose = TRUE, plot = TRUE
)
...
dec_df <- data.frame(
  x = numeric(), y = numeric(), kernel = character(),
  test = character()
)
...
for (i in 1:length(dis_list)) { # nolint
  dissims <- dis_list[[i]]
  for (j in 1:length(dissims)) { # nolint
    my_dissim <- dissims[[j]]
